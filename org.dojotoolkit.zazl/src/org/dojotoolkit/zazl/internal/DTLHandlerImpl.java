...
public class DTLHandlerImpl implements DTLHandler {
	...
	public DTLHandlerImpl(ResourceLoader dtlResourceHandler,
		...
		logger.logp(Level.FINER, DTLHandlerImpl.class.getName(), "DTLHandlerImpl", "useV8 = "+useV8 + " debug = "+debug+" v8Available = "+V8DTLHandler.v8Available+" useCache = "+useCache);
	}
	...
	public boolean handle(DTLContext dtlContext) throws IOException {
		...
		if (useV8 && V8DTLHandler.v8Available) {
			V8DTLHandler v8DTLHandler = new V8DTLHandler(dtlResourceHandler, dtlContext.getResponseHandler(), dtlContext.getCallbackHandler(), dtlContext.getCallbackNames(), useCache); 
			handled = v8DTLHandler.runDTLScript(dtlContext.getEnv(), dtlContext.getTemplateMapping());
		}
		else {
			RhinoDTLHandler rhinoDTLHandler = new RhinoDTLHandler(dtlContext.getResponseHandler(), dtlResourceHandler, debug, dtlContext.getCallbackHandler(), dtlContext.getCallbackNames(), rhinoClassLoader, useCache, debugContextFactory);
